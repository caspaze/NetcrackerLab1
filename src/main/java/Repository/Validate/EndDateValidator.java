...
public class EndDateValidator implements Validator {
    ...
    public Message validate(Contract contract) {
        ...
        if(contract.getContractStartDate().isBefore(LocalDate.now())){
            return new Message(Status.WARNING,"The date is in the past");
        }
        return new Message(Status.OK,"The end date is available");
    }
}
